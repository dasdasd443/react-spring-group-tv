...
const Header = (props) =>
{
    ...
    let user = (loggedIn !== false)? <div style={{display:"flex", gap: "1rem"}}> <Link to="/user/dashboard" className="user">{loggedIn.details.email}</Link> | <Link to="/" className="user" onClick={LogoutUser}>Logout</Link></div>: <Link to="/login" className="user">My profile</Link>;
